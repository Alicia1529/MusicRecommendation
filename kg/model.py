...
class RippleNet(object):
    ...
    def _build_inputs(self):
        self.batch_size = tf.placeholder(dtype=tf.int32, shape=(), name="bsize")
    ...
    def eval(self, sess, feed_dict):
        batch_size, labels, scores = sess.run([self.batch_size, self.labels, self.scores_normalized], feed_dict)
        auc = roc_auc_score(y_true=labels, y_score=scores)

        # add top k precision/recall/f1
        precision_K = []
        recall_K = []
        F1_K = []
        ...
        return auc, acc, precision_K, recall_K
